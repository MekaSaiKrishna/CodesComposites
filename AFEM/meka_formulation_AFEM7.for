...
C ****************  END FAILURE CRITERIA WRAPPER ***********************
      ...
      ! local crack stress
      statev(25) = sigcr0 !initialization to STRENGTH (XT)
      end if
...
C     STEP-6: If Crack Exists
      ...
      dEcr     = statev(21:23)  ! CURRENT INCREMENT IN LOCAL CRACK STRAIN  
...
C       Dcr(3,3) = Dcr(1,1)*term*1.0D-5
      ...
      CALL calcDcr(Ecr_OLD,Ecr_MAX,GIc,sigcr0,L,nu,Dcr,scr_old,scr,dEcr)
      statev(25) = scr
...
C     STEP-8: Determine local crack strain (increment) consistently
      ...
      CALL calcEcr(Ecr_OLD,Ecr_MAX,DSTRAN,Dco,N,dEcr,Ecr,GIc,sigcr0,L,nu,scr_old,scr)
...
C     !Calculate the Dcr matrix based on previous Ecr value and 
      !Ecr_MAX value
C       call calcDcr(Ecr_OLD,Ecr_MAX,GIc,sigcr0,L,nu,Dcr)
      call calcDcr(Ecr_OLD,Ecr_MAX,GIc,sigcr0,L,nu,Dcr,scr_old,scr,dEcr)
...
C-----------------------------------------------------------------------   
      subroutine calcDcr(Ecr_OLD,Ecr_MAX,GIc,sigcr0,L,nu,Dcr,scr_old,scr,dEcr)
      implicit none
      ...
      scr_max = sigcr0 + (-(sigcr0)**2/(2.0D0*GIc/L))*Ecr_MAX(1)
