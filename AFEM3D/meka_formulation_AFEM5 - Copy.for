...
C***********************************************************************
C COMPUTING EIGEN VALUES AND VECTORS
      Amat = (/ 4, 3, 2, 2, 0, 1 /)

      call mysprind(Amat,Pval,eigV,1,3,3)
C       write(*,*) 'eigV - col1: ',eigV(1,1),eigV(2,1),eigV(3,1)
      
      write(*,*) 'Pval(1): ', Pval(1)
      write(*,'(A,/,3F12.6)') 'eigV - row1: ',eigV(1,1),eigV(1,2),eigV(1,3) 
...
C     First time when strain overshoots, CUTBACK to the Peak Point
            IF ((EPS(1) + DSTRAN(1)) .GE. EPSN) THEN
...
C                   DEPS1       = EPS_STAR-EPS(1)
C                   DEPS2       = DSTRAN(1) - DEPS1
C                   SIG_CORR    = SIG_COMP + ALF2*(DEPS2)

                  DEPS1       = EPS_STAR-EPS(1)
C                   DEPS2       = DSTRAN(1) - DEPS1
                  SIG_CORR    = SIG_COMP - ALF2*(DEPS1)

                  STATEV(6) = SIG_CORR

                  ! Stress updated to Corrected stress before overshoot
C                   SIG(1)   = STATEV(6)
            END IF 

C-----------------------------------------------------------------------
      ELSE IF ((EPS(1) .GE. EPSN) .AND. (EPS(1) .LT. EPSF)) THEN

            Dco(1,1)    = ALF2
C             SIG         = SIG + MATMUL(Dco,DSTRAN)

            SIG(1)      = STATEV(6) + (Dco(1,1)*DSTRAN(1))
            STATEV(6)   = SIG(1)
...
C-----------------------------------------------------------------------
C     First time when strain overshoots, CUTBACK to the Failure Point
            IF ((EPS(1) + DSTRAN(1)) .GE. EPSF) THEN
...
C       STATEV(1) = isCRACKED 
C       STATEV(2) = MODE

C C     STORE THE PEAK STRESS VALUE OF TRACTION SEPARATION LAW
C       STATEV(7) = DMAX1(PEAK,STATEV(7))

C C     STORE THE MAX STRAIN OBSERVED 
C       STATEV(3) = DMAX1(EPS(1),EPS_MAX)

C C     BEFORE FAILURE -- LINEAR ELASTIC RESPONSE
C       IF (isCRACKED .LT. 0.5) THEN
C             CALL STIFFNESS_MATRIX(E,nu, Dco)
C             SIG = MATMUL(Dco,EPS)
C       END IF
