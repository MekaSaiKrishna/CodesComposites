...
C             write(*,*) 'EPS_OLD: ',EPS_OLD     ! PREVIOUS  STRAIN
            IF (MODE .EQ. 1) THEN
                  ...
                  IF (EPS(1) .LE. (2.0d0*G1C_F/XT)) THEN
                        ! POST-PEAK
                        IF (STATEV(3) .GE. XT/E) THEN
                              ...
                              IF (EPS(1) .EQ. STATEV(3)) THEN 
                                    ...
                                    PEAK = STATEV(7)
                                    ! Slope in Zone-2
                                    SLOPE = -1.0d0*PEAK/((2.0d0*G1C_F/PEAK)-(PEAK/E))
...
C                               ELSE IF ((EPS(1) .LT. STATEV(3)) .AND. (EPS(1) .LE. EPS_OLD)) THEN
                              ELSE IF (EPS(1) .LT. STATEV(3)) THEN
                                    ZONE = 3
                                    STATEV(4) = ZONE
